...
class ONet:
    ...
    def model_fn(self, features, labels, mode, params=None):
        if mode == ModeKeys.TRAIN:
            ...
            return self.mode_gen_sample_fn(mode, features, params)

    def onet(self, images, is_training=False, scope='onet'):
        with tf.name_scope(scope):
            with slim.arg_scope([slim.conv2d],
                ...
                with slim.arg_scope([slim.batch_norm], is_training=is_training):
                    net = slim.conv2d(images, num_outputs=32, kernel_size=[3, 3], stride=1)
                    net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, padding='SAME')
                    net = slim.conv2d(net, num_outputs=64, kernel_size=[3, 3], stride=1)
                    net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2)
                    net = slim.conv2d(net, num_outputs=64, kernel_size=[3, 3], stride=1)
                    net = slim.max_pool2d(net, kernel_size=[2, 2], stride=2, padding='SAME')
                    net = slim.conv2d(net, num_outputs=128, kernel_size=[2, 2], stride=1)
                    fc_flatten = slim.flatten(net)
                    fc1 = slim.fully_connected(fc_flatten, num_outputs=256)
    ...
    def model_train_fn(self, mode, features, params):
        ...
        predictions = self.onet(images, is_training=True)
        pred_cls, pred_loc = predictions['pred_cls'], predictions['pred_loc']

        optimizer = tf.train.AdamOptimizer(learning_rate=tfe.lr_plateau_decay())
        ...
        #summary
        metrics = self.metrics_fn(gt_cls, pred_cls, match_thres=thres)
        for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
            tf.summary.histogram(var.name, var)
        return tf.estimator.EstimatorSpec(mode, predictions, total_loss, train_op)

    def mode_gen_sample_fn(self, mode, features, params):
        assert mode == ModeKeys.PREDICT
        sample_imgs = features['sample_imgs']
        sample_bboxes = features['sample_bboxes']
        thres = params['thres']

        predictions = tf.cond(pred = tf.equal(tf.size(sample_imgs), 0),
                              true_fn = lambda: {'pred_cls':tf.zeros([1]), 'pred_logit':tf.zeros([1,2]),'pred_loc':tf.zeros([1,4]),},
                              false_fn = lambda: self.onet(sample_imgs, is_training=False))
        pred_cls, pred_loc = predictions['pred_cls'], predictions['pred_loc']
    ...
    def loss_fn(self, gt_cls, gt_loc, pred_cls, pred_loc, match_thres):
        with tf.name_scope('loss_fn'):
            ...
            pos_loss = tf.losses.mean_squared_error(tf.boolean_mask(gt_cls, pmask),
                                                    tf.boolean_mask(pred_cls, pmask))
            neg_loss = tf.losses.mean_squared_error(tf.boolean_mask(gt_cls, nmask),
                                                    tf.boolean_mask(pred_cls, nmask))
            loc_loss = tf.losses.mean_squared_error(tf.boolean_mask(gt_loc, pmask),
                                                    tf.boolean_mask(pred_loc, pmask))
